...
def dirtyTranslate(data):
    ...
    try:
        driver.get("https://www.deepl.com/en/translator#en/fr/stuff")
        inputer = driver.find_element_by_class_name('lmt__source_textarea')
        inputer.clear()
        inputer.send_keys(html.escape(data))
        WebDriverWait(driver, 40).until(
            ExpectedConditions.text_to_be_present_in_element_value((By.CLASS_NAME, 'lmt__target_textarea'), "p"))
        output = driver.find_element_by_class_name('lmt__target_textarea')
        transData = html.unescape(output.get_attribute("value"))
        driver.quit()
